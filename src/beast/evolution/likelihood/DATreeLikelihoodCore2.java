 ...
 */
@Deprecated
public class DATreeLikelihoodCore2 extends AbstrDATreeLikelihoodCore2 {
...
//    protected int[] storedBrLdIndex;
    ...
    public DATreeLikelihoodCore2(int nrOfStates, NodesStatesAndTree nodesStates, int categoryCount) {
        super(nrOfStates, nodesStates, categoryCount);
    } // called initialize()
...
//        this.nrOfCategories = categoryCount;
        ...
        if (getNrOfInterNodes() != getNrOfTips() - 1)
            throw new IllegalArgumentException("Internal nodes " + getNrOfInterNodes() +
                    " != tips " + getNrOfTips() + " - 1 !");
...
//        states = null;
    ...
    public void setInternalNodeStates(int nodeIndex, int[] states) {
        assert nodeIndex >= getNrOfTips();
        nodesStates.setNodeStates(nodeIndex, states);
    }

    @Override
    public int[] getNodeStates(int nodeIndex) {
        return nodesStates.getNodeStates(nodeIndex); // pass reference
    }
...
//    }
    ...
    public void calculateNodeBrLdOverCategories(int nodeChild1, int nodeParent, double[] proportions) {
        final int[] node1States = nodesStates.getNodeStates(nodeChild1);
        final int[] node3States = nodesStates.getNodeStates(nodeParent);
