...
//
//    /**
//     * Parsimony to init ancestral states (Cunningham et. al. 1998).
//     * Randomly choose a state from the ambiguous set with equal prob.
//     * No ambiguous ancestral states.
//     * @return ancestral states int[][].
//     *         Rows are the internal node array index, which is nodeNr - tipsCount.
//     *         Columns are sites.
//     */
...
//            int nr = tree.getRoot().getNr();
//            // use the root state assigned in down pass to start
//            uppass1SiteMap.put(nr, downpass1SiteMap.get(nr));
//            traverseAwayRoot(tree.getRoot(), k, uppass1SiteMap);
//
...
//    protected Set<Integer> getStatesFrom(Node node, final int siteNr,
...
//    protected void assignAncestralStates(Node node1, Node node2, Node node3, final int siteNr,
//                                       Map<Integer, Set<Integer>> anceStates1SiteMap) {
...
//    //
//    public void traverseToRoot(Node node, final int siteNr, Map<Integer, Set<Integer>> downpass1SiteMap) {
//        // Traverse down the two child nodes
//        final Node child1 = node.getChild(0);
//        if (!child1.isLeaf()) traverseToRoot(child1, siteNr, downpass1SiteMap);
//
//        final Node child2 = node.getChild(1);
//        if (!child2.isLeaf()) traverseToRoot(child2, siteNr, downpass1SiteMap);
//
//        assignAncestralStates(child1, child2, node, siteNr, downpass1SiteMap);
//    }
//
//    // uppass optimization away from the root
//    // input nodes are the ancestor and its sister node.
//    public void traverseAwayRoot(Node node, final int siteNr, final Map<Integer, Set<Integer>> downpass1SiteMap,
//                                 Map<Integer, Set<Integer>> uppass1SiteMap) {
//        // use the root state assigned in down pass to start
//        if (node.isRoot()) {
//            int nr = node.getNr();
//            uppass1SiteMap.put(nr, downpass1SiteMap.get(nr));
...
//        final Node child2 = node.getChild(1);
//
//        assignAncestralStates(child1, child2, node, siteNr, uppass1SiteMap);
//
//        if (!child1.isLeaf()) traverseAwayRoot(child1, siteNr, downpass1SiteMap, uppass1SiteMap);
//        if (!child2.isLeaf()) traverseAwayRoot(child2, siteNr, downpass1SiteMap, uppass1SiteMap);
//    }
//
//
