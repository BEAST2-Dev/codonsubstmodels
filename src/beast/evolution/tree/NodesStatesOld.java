 ...
 */
@Deprecated
public class NodesStatesOld extends StateNode {
     ...
     */
    public NodesStatesOld(CodonAlignment codonAlignment) {
     ...
     */
    public NodesStatesOld(int stateCount, int nodeCount, int siteCount, GeneticCode geneticCode) {
    ...
    public void initAndValidate() {
        ...
        // init from constructor
        NodesStatesOld nodesStates = new NodesStatesOld(stateCount, nodeCount, siteCount, geneticCode);
        // set tips states after initParam
    ...
    public void setTipStates(CodonAlignment codonAlignment) {
        ...
        for (int taxonIndex = 0; taxonIndex < tipsCount; taxonIndex++) {
            ...
            assert statesList.size() == states[currentMatrixIndex[taxonIndex]][taxonIndex].length;
            // Java 8
...
//        }
    ...
    public StateNode copy() {
        try {
            ...
            final NodesStatesOld copy = (NodesStatesOld) this.clone();
//            copy.states = states.clone(); // always [2][][]
    ...
    public void assignTo(StateNode other) {
        @SuppressWarnings("unchecked")
        final NodesStatesOld copy = (NodesStatesOld) other;
        copy.setID(getID());
        copy.index = index;
        for (int i = 0; i < getNodeCount(); i++) {
            copy.states[0][i] = states[0][i].clone();
    ...
    public void assignFrom(StateNode other) {
        @SuppressWarnings("unchecked")
        final NodesStatesOld source = (NodesStatesOld) other;
        setID(source.getID());
