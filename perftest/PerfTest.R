...
getBrLenESSHour <- function(tre.log="m0.trees", screen.log="out.txt", burnin=0.1) {
  ...
  # read screen log
  screen.log <- file.path(paste0("s4t",nTaxa), paste0("t",nTaxa,"st.xml_out.txt"))
...
screen.log <- file.path(paste0("4t",nTaxa), paste0("t",nTaxa,"th4.xml_out.txt"))
...
p <- ggplot(data.m, aes(method, value)) + 
  ...
  theme_minimal()
ggsave(paste0("t",nTaxa,"-ess-hour.pdf"), p, width = 5, height = 5)
